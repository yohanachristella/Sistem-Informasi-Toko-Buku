# Sistem-Informasi-Toko-Buku
Program POS(Point of Sales) Penjualan Buku, didevelop menggunakan Visual Basic. Program POS ini terdiri dari beberapa bagian utama, seperti tampilan Best seller book, master buku, master member, user, dan supplier, penjualan, supply, serta laporan yang diintegrasikan dengan Crystal Report. Program POS ini juga diintegrasikan dengan database mySQL.
